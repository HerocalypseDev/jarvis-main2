...
## Corpus Check
- 84 files · ~223,496 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 4 file(s) not represented in the graph (top: (none) 1, .vbs 1, .css 1)

## Summary
- 3203 nodes · 6950 edges · 146 communities (131 shown, 12 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 371 edges (avg confidence: 0.85)
...
Cohesion: 0.09
Nodes (44): click_at(), _current_command_source(), drag_and_drop(), _execute_tool_impl(), focus_window(), list_background_tasks(), list_reminders(), _queue_pending_confirmation() (+36 more)
...
Cohesion: 0.04
Nodes (87): _acquire_single_instance_lock(), _away_warn(), block_samples(), pending(), _bytes_to_gb(), _bytes_to_mb(), check_system_health(), _choose_input_device() (+79 more)

### Community 7 - "queue_or_deliver_notification"
Cohesion: 0.11
Nodes (27): _check_due_reminders(), notify(), _failover_to_gemini(), _finish_background_task(), flush_pending_notifications(), True after the owner said "no": reminders speak even with a stranger in view., reminders_allowed(), queue_or_deliver_notification() (+19 more)
...
Cohesion: 0.05
Nodes (39): AbstractEventLoop, _autonomy_create_event(), _autonomy_inbound(), _autonomy_mail_hook(), mail(), _dashboard_get_services_status(), _ensure_mcp_loop(), ensure_mcp_started() (+31 more)
...
Cohesion: 0.07
Nodes (8): _gate_env(), _kv_db(), Tests for the caching layers (jarvis_cache.py + their wiring in jarvis.py). Run…, test_billing_summary_and_pagination(), test_busy_gate_holds_ordinary_notification_but_not_reminders(), test_bypass_busy_gate_still_respects_sleep_mode(), test_record_and_local_summary(), test_short_notification_is_spoken_unchanged_without_a_claude_call()
...
Cohesion: 0.18
Nodes (11): _autonomy_calendar_events(), _autonomy_callbacks(), _autonomy_run_agent(), _commands_in_flight(), _get_idle_seconds(), Seconds since the last system-wide keyboard/mouse input, via GetLastInputInfo.…, True if the user touched the keyboard/mouse within ACTIVE_IDLE_THRESHOLD_S…, Runs one approved autonomous action through the normal agent loop (so its… (+3 more)
...
Cohesion: 0.09
Nodes (23): _cancel_media_autopause(), _db_path(), _endpoint_volume_call(), fish_audio_prosody_overrides(), _get_volume(), guided_breathing_steps(), is_whitelisted_sender(), _log_duration_to_memory() (+15 more)
...
### Community 31 - "Path"
Cohesion: 0.07
Nodes (34): _background_tasks_dir(), _catastrophic_reason(), _check_background_tasks(), _cleanup_old_logs(), _delegate_child_env(), _delegate_research(), _run(), _delegate_to_claude_code() (+26 more)
...
Cohesion: 0.11
Nodes (26): calendar_items(), compose(), deadline_items(), _hm(), mail_items(), datetime, Morning briefing v2 and "what's urgent?" (2026-09-23). One composition used by…, Calendar MCP (@cocal/google-calendar-mcp) list-events JSON -> "9:30 AM Standup"… (+18 more)
...
Cohesion: 0.06
Nodes (44): _clipboard_has_non_text(), _dictate(), _get_whisper_model(), _grab_selection(), handle_text_command(), handle_voice_command(), _handle_voice_command_impl(), _hold_modes() (+36 more)
...
Cohesion: 0.13
Nodes (13): ndarray, Request, Deepgram Nova-3 STT (Speed Upgrade Phase 1.1, streaming added in the cloud-…, Connection warm-up (Phase 2.2): a near-silent clip so the TLS handshake happens…, Hard-timeout watchdog: urlopen's own timeout doesn't reliably fire on a wedged…, transcribe(), _urlopen_bounded(), warm() (+5 more)
...
Cohesion: 0.08
Nodes (26): RuntimeError, test_tick_hands_slow_steps_to_bounded_workers_and_survives_a_failing_step(), test_record_usage_never_raises(), broken(), test_daily_endpoint_exception_does_not_break(), boom(), test_get_pending_exception_does_not_break_state(), boom() (+18 more)
...
Cohesion: 0.26
Nodes (17): disable(), enable(), _get_state(), is_active(), kind='nap' runs the exact same mode (quiet notifications, mail take-over, dark…, _set_volume(), toggle(), _fake_env() (+9 more)
...
Cohesion: 0.15
Nodes (18): _dashboard_get_sleep(), _connect(), _hhmm(), _period_stats(), Connection, Stats over the `days` calendar days ending at end_day (inclusive): tracked…, Read-only sleep trends for the dashboard, from sleep_log alone (no new tables).…, save_digest() (+10 more)
...
Cohesion: 0.05
Nodes (49): _append_history(), _apply_memory_db_pragmas(), _briefing_fetchers(), calendar(), failed(), reminders(), _calendar_events_raw(), cancel_reminder() (+41 more)
...
Cohesion: 0.14
Nodes (15): briefing_report(), _deterministic_intent_reply(), _last_actions_reply(), _parse_duration_s(), Morning briefing ("morning") or needs-you summary ("urgent"): {"sections":…, Local handling for timers and the stopwatch; None = let the agent loop handle…, Zero-LLM-call answers for the handful of intents that are pure local…, _recent_actions() (+7 more)

### Community 129 - "run_agent_loop"
Cohesion: 0.05
Nodes (53): enabled(), JARVIS_<LAYER>_CACHE env flag, default on. Read on every call so it can be…, record(), stable_hash(), _cached_tools(), _collapse_paths_for_speech(), _dashboard_approve_pending(), _sink() (+45 more)
...
Cohesion: 0.04
Nodes (59): system(), _build_sleep_digest(), _claude_failure_reason(), _claude_in_cooldown(), _claude_live_problem(), _claude_request(), _claude_stream_first_round(), _speak_ready() (+51 more)
...
- **Why does `_execute_tool_impl()` connect `_execute_tool_impl` to `run_agent_loop`, `test_sleep.py`, `FileWatcher`, `jarvis.py`, `queue_or_deliver_notification`, `jarvis_memory_enhance.py`, `ensure_mcp_started`, `jarvis_guest_reminders.py`, `_claude_request`, `jarvis_dynamic_tools.py`, `jarvis_workflow.py`, `jarvis_proactive.py`, `jarvis_focus.py`, `jarvis_sleep_mode.py`, `download_image`, `jarvis_autonomy_organise.py`, `Path`, `_handle_text_command_impl`, `test_qol.py`, `test_restart.py`, `test_hardening.py`, `jarvis_billing.py`, `jarvis_tech_understanding.py`, `_db`, `jarvis_face.py`, `jarvis_autonomy_skills.py`, `test_face.py`, `build_system_blocks`, `_rows`, `is_paused`, `enable`, `test_dev_features.py`, `jarvis_task_scheduler.py`, `_memory_db_connect`, `test_guest_reminders.py`, `_deterministic_intent_reply`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `_build_app()` connect `_build_app` to `start`, `test_dashboard_llm_endpoints`, `_ConnectionManager`, `_set_broadcast`, `_origin_is_loopback`, `_provider`, `jarvis_dashboard.py`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
- **Why does `j()` connect `j` to `is_paused`, `test_hardening.py`, `test_sleep.py`, `_FakeProc`, `test_gemini.py`, `test_sleep_mail.py`, `CircuitBreaker`, `test_guest_reminders.py`, `test_hold_modes.py`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
