...
## Corpus Check
- 94 files · ~233,953 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 4 file(s) not represented in the graph (top: (none) 1, .vbs 1, .css 1)

## Summary
- 3359 nodes · 7261 edges · 153 communities (142 shown, 8 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 403 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `db2bc78a`
...
Cohesion: 0.05
Nodes (61): _bytes_to_gb(), _bytes_to_mb(), check_system_health(), _choose_input_device(), _chrome_executable(), _craft_system_status_summary(), _cursor_executable(), _cursor_foreground_hwnd_win32() (+53 more)

### Community 7 - "run_agent_loop"
Cohesion: 0.04
Nodes (80): _autonomy_callbacks(), _autonomy_run_agent(), build_system_blocks(), enabled(), is_self_contained(), normalize_text(), Small, local-only cache helpers shared by jarvis.py: an env-flag reader,…, JARVIS_<LAYER>_CACHE env flag, default on. Read on every call so it can be… (+72 more)
...
Cohesion: 0.13
Nodes (14): ensure_mcp_started(), _load_mcp_server_configs(), _mcp_connect_all_async(), _mcp_connect_one(), _mcp_server_supervisor(), _mcp_servers_config_path(), _McpServerHandle, _preload_mcp_async() (+6 more)
...
### Community 35 - "main"
Cohesion: 0.06
Nodes (35): _acquire_single_instance_lock(), _away_warn(), block_samples(), _dashboard_get_sleep(), _dashboard_get_usage(), _dashboard_kill_background_task(), _face_greet(), Queue non-urgent notifications while Focus Mode is on; urgent ones always get… (+27 more)
...
Cohesion: 0.07
Nodes (38): _clipboard_has_non_text(), _grab_selection(), handle_text_command(), handle_voice_command(), _handle_voice_command_impl(), _inflight_enter(), _inflight_exit(), classify_intent() (+30 more)
...
Cohesion: 0.11
Nodes (25): duck(), enabled(), _mute_others(), _pause_music(), _pause_pattern(), Mute every other app's audio while Jarvis speaks, and unmute it afterwards.…, Call when Jarvis's audio starts. Nested/overlapping calls are counted., Call when Jarvis's audio ends; other apps are unmuted `delay` seconds after the… (+17 more)
...
### Community 95 - "state"
Cohesion: 0.25
Nodes (20): answer(), has_open_question(), holding_reminders(), on_stranger_arrived(), on_stranger_left(), Handle a reply to an open question. Returns the reply to send back, or None if…, True while due reminders must be held (not spoken, no toast)., state() (+12 more)

### Community 96 - "stats_summary"
Cohesion: 0.20
Nodes (14): _connect(), _hhmm(), _period_stats(), Connection, Stats over the `days` calendar days ending at end_day (inclusive): tracked…, Read-only sleep trends for the dashboard, from sleep_log alone (no new tables).…, save_digest(), set_wake_digest_handler() (+6 more)
...
Cohesion: 0.05
Nodes (46): _append_history(), _apply_memory_db_pragmas(), _briefing_fetchers(), failed(), reminders(), build_system_prompt(), cancel_reminder(), _create_memory_tables() (+38 more)

### Community 117 - "jarvis_sleep_mode.py"
Cohesion: 0.11
Nodes (19): _cancel_media_autopause(), _db_path(), _endpoint_volume_call(), _get_volume(), guided_breathing_steps(), is_whitelisted_sender(), _log_duration_to_memory(), play_ambient() (+11 more)
...
### Community 124 - "_deterministic_intent_reply"
Cohesion: 0.09
Nodes (23): pending(), briefing_report(), _dashboard_get_pending(), _deterministic_intent_reply(), _face_release_held_notifications(), flush_pending_notifications(), health_report(), _interrupt_speech() (+15 more)
...
### Community 126 - "_delegate_to_claude_code"
Cohesion: 0.08
Nodes (32): _background_tasks_dir(), _catastrophic_reason(), _check_background_tasks(), _count_running_background_tasks(), _current_command_source(), _dashboard_approve_pending(), _sink(), notify() (+24 more)

### Community 127 - "jarvis_roblox.py"
Cohesion: 0.18
...
Cohesion: 0.04
Nodes (62): HTMLParser, system(), _build_sleep_digest(), _claude_failure_reason(), _claude_in_cooldown(), _claude_live_problem(), _claude_request(), _claude_stream_first_round() (+54 more)
...
Cohesion: 0.04
Nodes (75): AbstractEventLoop, _autonomy_calendar_events(), _autonomy_create_event(), _autonomy_inbound(), _autonomy_mail_hook(), _autonomy_poll_mail(), calendar(), mail() (+67 more)
...
### Community 146 - "jarvis_untrusted.py"
Cohesion: 0.40
...
- **Why does `_execute_tool_impl()` connect `_execute_tool_impl` to `test_sleep.py`, `FileWatcher`, `jarvis.py`, `run_agent_loop`, `test_dev_features.py`, `queue_or_deliver_notification`, `jarvis_memory_enhance.py`, `_claude_request`, `jarvis_guest_reminders.py`, `jarvis_workflow.py`, `jarvis_dynamic_tools.py`, `jarvis_vibes.py`, `jarvis_proactive.py`, `jarvis_weather.py`, `download_image`, `jarvis_autonomy_organise.py`, `Path`, `test_restart.py`, `test_hardening.py`, `jarvis_billing.py`, `jarvis_tech_understanding.py`, `is_paused`, `jarvis_face.py`, `jarvis_autonomy_skills.py`, `test_face.py`, `_load_skills`, `jarvis_autonomy.py`, `_db`, `jarvis_focus.py`, `jarvis_task_scheduler.py`, `_memory_db_connect`, `jarvis_sleep_mode.py`, `enable`, `test_guest_reminders.py`, `_deterministic_intent_reply`, `_delegate_to_claude_code`, `jarvis_roblox.py`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `j()` connect `j` to `test_hardening.py`, `test_timers.py`, `test_memory_edit.py`, `test_sleep.py`, `test_voice_usage.py`, `is_paused`, `_FakeProc`, `test_gemini.py`, `test_sleep_mail.py`, `test_chief.py`, `CircuitBreaker`, `test_guest_reminders.py`, `test_safe_mode.py`, `test_hold_modes.py`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Why does `_future()` connect `Fake` to `test_autonomy.py`, `timedelta`, `_cb`, `RuntimeError`, `_acted`, `_tools`?**
