...
## Corpus Check
- 97 files · ~240,738 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 4 file(s) not represented in the graph (top: (none) 1, .vbs 1, .css 1)

## Summary
- 3454 nodes · 7450 edges · 169 communities (153 shown, 13 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 414 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `22b0c9da`
...
Cohesion: 0.09
Nodes (45): click_at(), _current_command_source(), drag_and_drop(), _execute_tool_impl(), focus_window(), _is_whatsapp_web_url(), list_background_tasks(), list_reminders() (+37 more)
...
Cohesion: 0.10
Nodes (14): db(), jarvis(), _log(), fixture, Tests for sleep trend stats and the wake-up digest. Run with: python -m pytest…, A .env setting read at import time (sleep goal) must reach the modules…, test_digest_important_first_then_lighter_note(), test_disable_calls_digest_handler_and_digest_is_saved() (+6 more)
...
Cohesion: 0.04
Nodes (83): _background_tasks_dir(), _budget_check(), _bytes_to_gb(), _bytes_to_mb(), _check_background_tasks(), check_system_health(), _chief_tick(), _choose_input_device() (+75 more)

### Community 7 - "self_check_report"
Cohesion: 0.12
Nodes (19): _claude_failure_reason(), _claude_live_problem(), _fish_audio_synthesize(), api_key(), _gemini_live_problem(), model_name(), _invalidate_read_caches(), _llm_status() (+11 more)
...
Cohesion: 0.09
Nodes (21): _broadcast_theme_change(), _cancel_media_autopause(), _dark_mode_is_on(), fish_audio_prosody_overrides(), guided_breathing_steps(), is_whitelisted_sender(), _log_duration_to_memory(), play_ambient() (+13 more)
...
Cohesion: 0.18
Nodes (17): IPv4Network, _hostnames(), is_private_mac(), local_ip(), normalize_mac(), parse_arp(), parse_ipconfig(), _run() (+9 more)
...
Cohesion: 0.07
Nodes (29): HTMLParser, _build_sleep_digest(), _claude_request(), _claude_stream_first_round(), _speak_ready(), _claude_text(), _duckduckgo_search(), _DuckDuckGoResultParser (+21 more)
...
### Community 45 - "_autonomy_callbacks"
Cohesion: 0.09
Nodes (24): _autonomy_calendar_events(), _autonomy_callbacks(), _autonomy_create_event(), _autonomy_run_agent(), calendar(), _calendar_events_raw(), _commands_in_flight(), create_reminder() (+16 more)
...
Cohesion: 0.10
Nodes (45): group_safe(), group_safe_suppress(), list_snapshots(), poll_once(), _Presence, One recognition cycle. Returns a short status word (used by tests and…, True while an unrecognized person is in view. Only ever used to hold *spoken…, One volatile-block line so the agent knows who is present. Personalization… (+37 more)
...
### Community 69 - "jarvis_sleep_mail.py"
Cohesion: 0.14
...
Cohesion: 0.04
Nodes (44): invalidate_profile_cache(), True while the Windows lock screen (secure desktop) is up: the camera is off-…, _session_locked(), away(), enrolled(), fx(), jarvis(), fixture (+36 more)
...
Cohesion: 0.07
Nodes (69): Exception, away_enabled(), away_grace_s(), _away_reset(), away_status(), _away_step(), away_warn_s(), _camera_failed() (+61 more)
...
### Community 77 - "enroll"
Cohesion: 0.09
Nodes (38): _already_enrolled(), _clean_name(), delete(), delete_by_id(), describe_profiles(), enroll(), list_profiles(), (role, None) to enroll with, or (None, reason). The first person is always the… (+30 more)

### Community 78 - "classify_intent"
Cohesion: 0.25
Nodes (9): classify_intent(), parametrize, test_briefing_intents(), parametrize, test_classify_intent(), parametrize, test_media_voice_commands(), test_new_intents() (+1 more)
...
Cohesion: 0.22
Nodes (10): _dashboard_get_daily_items(), get_skills_context(), _load_skills(), Skills from disk, re-parsed only when a skills/*.json file was added, removed…, Reads every *.json skill file from the skills directory. A malformed file is…, Writes name/description/instructions (and optional schedule) as a new…, Read-only snapshot for the dashboard's own Daily section (recurring skills +…, _read_skills_from_disk() (+2 more)
...
### Community 112 - "_collapse_paths_for_speech"
Cohesion: 0.13
Nodes (14): _autonomy_inbound(), _autonomy_mail_hook(), _collapse_paths_for_speech(), _humanize_path_for_speech(), Replaces any full file path in `text` with just its containing folder's name —…, Files something under the wake-up recap's important section without speaking it., Registered with jarvis_sleep_mode: when Sleep Mode ends, replaces the old flood…, While Sleep Mode is on, check Gmail every SLEEP_MAIL_INTERVAL_MIN minutes… (+6 more)

### Community 113 - "RuntimeError"
Cohesion: 0.10
Nodes (27): _data_dir(), _DataBlob, _db_path(), download_models(), _dpapi(), blob(), _encrypt(), _has_encrypted_data() (+19 more)
...
Cohesion: 0.05
Nodes (65): build_system_blocks(), build_system_prompt(), enabled(), JARVIS_<LAYER>_CACHE env flag, default on. Read on every call so it can be…, record(), stable_hash(), _cached_tools(), _execute_tool() (+57 more)
...
### Community 124 - "jarvis_focus.py"
Cohesion: 0.13
Nodes (23): _face_greet(), classify_mood(), current_track(), disable(), enable(), _enabled_flag(), is_active(), Focus Mode, optionally triggered by the mood of what Spotify is playing. Mood… (+15 more)
...
Cohesion: 0.22
Nodes (20): disable(), enable(), _get_state(), is_active(), kind='nap' runs the exact same mode (quiet notifications, mail take-over, dark…, _set_state(), set_system_action_handler(), toggle() (+12 more)

### Community 129 - "_meeting_headsup"
Cohesion: 0.19
...
Cohesion: 0.07
Nodes (29): _briefing_fetchers(), failed(), mail(), pending(), reminders(), system(), briefing_report(), _claude_in_cooldown() (+21 more)
...
### Community 136 - "record"
Cohesion: 0.24
Nodes (8): describe(), _ensure(), Connection, Store a scan; returns the devices never seen before on this network (empty on…, record(), Network devices card (jarvis_netscan): parsing, new-device detection, route.…, _result(), test_first_scan_is_silent_baseline_then_new_macs_are_reported()

### Community 137 - "queue_or_deliver_notification"
Cohesion: 0.12
Nodes (24): _check_due_reminders(), notify(), _failover_to_gemini(), _finish_background_task(), flush_pending_notifications(), True after the owner said "no": reminders speak even with a stranger in view., reminders_allowed(), queue_or_deliver_notification() (+16 more)
...
Cohesion: 0.06
Nodes (39): _clipboard_has_non_text(), _dictate(), _grab_selection(), handle_text_command(), handle_voice_command(), _handle_voice_command_impl(), _inflight_enter(), _inflight_exit() (+31 more)
...
Cohesion: 0.14
Nodes (18): _dashboard_get_sleep(), _connect(), _db_path(), _hhmm(), _period_stats(), Connection, Path, Stats over the `days` calendar days ending at end_day (inclusive): tracked… (+10 more)

### Community 146 - "jarvis_voice_tone.py"
Cohesion: 0.32
...
### Community 148 - "test_briefing.py"
Cohesion: 0.25
Nodes (6): Morning briefing v2 / "what's urgent?" (jarvis_briefing). No network, temp DB…, test_briefing_report_reads_real_reminders_and_pending(), test_calendar_items_parse_mcp_json_and_flag_overlaps(), test_compose_omits_empty_failed_and_slow_sections(), boom(), test_speech_caps_items_and_length()

### Community 149 - "calibrate"
Cohesion: 0.13
Nodes (13): calibrate(), camera_index(), CameraUnavailable, _capture_and_analyze(), _get_engine(), liveness_min_swing(), _open_camera(), Open the camera, grab one frame, release it. Returns (observations, mean, std,… (+5 more)
...
### Community 153 - "_play_pcm_stream"
Cohesion: 0.29
Nodes (5): _play_pcm_stream(), Attempts Deepgram's streaming speak WebSocket for `text`, playing audio as it's…, Plays int16 PCM chunks as they're produced by an iterable (a live WebSocket…, _speak_streamed(), _speech_cancelled_since()

### Community 154 - "_volume_reply"
Cohesion: 0.38
...
### Community 161 - "main"
Cohesion: 0.05
Nodes (41): _acquire_single_instance_lock(), _away_warn(), block_samples(), _dashboard_approve_pending(), _sink(), _dashboard_kill_background_task(), _dashboard_reject_pending(), _execute_confirmed_action() (+33 more)
...
- **Why does `_execute_tool_impl()` connect `_execute_tool_impl` to `enable`, `_deterministic_intent_reply`, `jarvis.py`, `FileWatcher`, `jarvis_memory_enhance.py`, `queue_or_deliver_notification`, `self_check_report`, `jarvis_sleep_mode.py`, `jarvis_guest_reminders.py`, `jarvis_workflow.py`, `jarvis_dynamic_tools.py`, `test_qol.py`, `stats_summary`, `jarvis_proactive.py`, `download_image`, `jarvis_autonomy_organise.py`, `Path`, `test_dev_features.py`, `_claude_request`, `test_restart.py`, `_autonomy_callbacks`, `test_hardening.py`, `jarvis_billing.py`, `jarvis_tech_understanding.py`, `jarvis_face.py`, `jarvis_autonomy_skills.py`, `enroll`, `_load_skills`, `_rows`, `jarvis_task_scheduler.py`, `_memory_db_connect`, `run_agent_loop`, `test_guest_reminders.py`, `jarvis_focus.py`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `_future()` connect `Fake` to `test_autonomy.py`, `_cb`, `timedelta`, `_tools`, `_mail_answer`?**
  _High betweenness centrality (0.042) - this node is a cross-community bridge._
- **Why does `j()` connect `j` to `test_hardening.py`, `test_voice_usage.py`, `test_sleep.py`, `test_face.py`, `_FakeProc`, `test_gemini.py`, `test_sleep_mail.py`, `test_memory_edit.py`, `test_chief.py`, `CircuitBreaker`, `test_timers.py`, `test_guest_reminders.py`, `test_safe_mode.py`, `test_hold_modes.py`?**
