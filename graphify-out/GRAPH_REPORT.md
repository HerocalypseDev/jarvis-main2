...
## Corpus Check
- 86 files · ~225,925 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 4 file(s) not represented in the graph (top: (none) 1, .vbs 1, .css 1)

## Summary
- 3231 nodes · 7004 edges · 145 communities (129 shown, 13 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 377 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0f42a5e4`
...
Cohesion: 0.05
Nodes (68): _broadcast_theme_change(), _cancel_media_autopause(), _connect(), _db_path(), disable(), enable(), _endpoint_volume_call(), _get_state() (+60 more)
...
Cohesion: 0.04
Nodes (94): _acquire_single_instance_lock(), _autonomy_inbound(), _autonomy_mail_hook(), block_samples(), _bytes_to_gb(), _bytes_to_mb(), check_system_health(), _choose_input_device() (+86 more)

### Community 7 - "queue_or_deliver_notification"
Cohesion: 0.05
Nodes (52): AbstractEventLoop, _check_due_reminders(), _ensure_mcp_loop(), _forward_held_reminders(), _get_idle_seconds(), _get_last_skill_run(), _guess_project_from_window_title(), forward_reminder() (+44 more)
...
Cohesion: 0.06
Nodes (44): _clipboard_has_non_text(), _dictate(), _grab_selection(), handle_text_command(), handle_voice_command(), _handle_voice_command_impl(), _inflight_enter(), _inflight_exit() (+36 more)
...
Cohesion: 0.07
Nodes (21): calibrate(), camera_index(), CameraUnavailable, _capture_and_analyze(), _get_engine(), liveness_min_swing(), _open_camera(), Open the camera, grab one frame, release it. Returns (observations, mean, std,… (+13 more)
...
Cohesion: 0.08
Nodes (20): _briefing_fetchers(), failed(), pending(), reminders(), system(), briefing_report(), _claude_in_cooldown(), _claude_stream_first_round() (+12 more)
...
Cohesion: 0.07
Nodes (35): _away_warn(), stable_hash(), _face_greet(), Queue non-urgent notifications while Focus Mode is on; urgent ones always get…, should_suppress(), _play_pcm_bytes(), _play_pcm_stream(), Silent cache lookup (no hit/miss stats recorded — the caller decides… (+27 more)
...
Cohesion: 0.06
Nodes (51): _autonomy_calendar_events(), _autonomy_callbacks(), _autonomy_create_event(), _autonomy_poll_mail(), _autonomy_run_agent(), calendar(), mail(), enabled() (+43 more)

### Community 139 - "_claude_request"
Cohesion: 0.06
Nodes (41): _build_sleep_digest(), _claude_failure_reason(), _claude_live_problem(), _claude_request(), _claude_text(), _duckduckgo_search(), _failover_to_gemini(), _fish_audio_synthesize() (+33 more)
...
- **Why does `_execute_tool_impl()` connect `_execute_tool_impl` to `speak_text`, `jarvis_window_control.py`, `jarvis_sleep_mode.py`, `FileWatcher`, `jarvis.py`, `run_agent_loop`, `queue_or_deliver_notification`, `jarvis_memory_enhance.py`, `_claude_request`, `jarvis_guest_reminders.py`, `jarvis_workflow.py`, `jarvis_dynamic_tools.py`, `jarvis_proactive.py`, `jarvis_weather.py`, `download_image`, `jarvis_autonomy_organise.py`, `Path`, `_handle_text_command_impl`, `test_qol.py`, `test_restart.py`, `poll_once`, `jarvis_billing.py`, `jarvis_tech_understanding.py`, `test_face.py`, `jarvis_face.py`, `jarvis_autonomy_skills.py`, `enroll`, `build_system_blocks`, `_rows`, `test_dev_features.py`, `jarvis_task_scheduler.py`, `test_guest_reminders.py`, `_briefing_fetchers`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `j()` connect `j` to `test_hardening.py`, `test_memory_edit.py`, `jarvis_sleep_mode.py`, `test_face.py`, `_FakeProc`, `test_gemini.py`, `test_sleep_mail.py`, `CircuitBreaker`, `test_guest_reminders.py`, `test_safe_mode.py`, `test_hold_modes.py`?**
