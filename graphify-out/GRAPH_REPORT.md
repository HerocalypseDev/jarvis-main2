...
## Corpus Check
- 92 files · ~232,569 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 4 file(s) not represented in the graph (top: (none) 1, .vbs 1, .css 1)

## Summary
- 3329 nodes · 7205 edges · 154 communities (140 shown, 11 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 392 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1f13e4ff`
...
Cohesion: 0.12
Nodes (36): click_at(), drag_and_drop(), _execute_tool_impl(), focus_window(), Runs one tool call and returns the text to feed back to Claude as its…, scroll_screen(), type_text(), arrange_windows() (+28 more)
...
Cohesion: 0.07
Nodes (27): _dashboard_get_sleep(), _hhmm(), _period_stats(), Stats over the `days` calendar days ending at end_day (inclusive): tracked…, Read-only sleep trends for the dashboard, from sleep_log alone (no new tables).…, stats_summary(), status(), db() (+19 more)
...
### Community 7 - "_autonomy_callbacks"
Cohesion: 0.08
Nodes (25): _autonomy_calendar_events(), _autonomy_callbacks(), _autonomy_create_event(), _autonomy_run_agent(), calendar(), _calendar_events_raw(), _commands_in_flight(), create_reminder() (+17 more)
...
Cohesion: 0.06
Nodes (14): _gate_env(), _kv_db(), Tests for the caching layers (jarvis_cache.py + their wiring in jarvis.py). Run…, Fake Claude: with a tool_name, the first call requests it and the next returns…, _scripted_claude(), test_busy_gate_holds_ordinary_notification_but_not_reminders(), test_bypass_busy_gate_still_respects_sleep_mode(), test_record_and_local_summary() (+6 more)
...
### Community 20 - "queue_or_deliver_notification"
Cohesion: 0.05
Nodes (56): dtime, mail(), _budget_check(), budget_alert(), headsup_text(), in_quiet_hours(), _local(), meetings_starting() (+48 more)
...
### Community 31 - "Path"
Cohesion: 0.13
Nodes (19): _cleanup_old_logs(), _fmt_gb(), get_large_files_report(), _memory_db_path(), _print_large_files_breakdown(), Path, _quarantine_corrupt_memory_db(), Renames a corrupted memory DB aside (never deletes) so a fresh one can take its… (+11 more)
...
Cohesion: 0.13
Nodes (14): _build_app(), api_briefing(), api_health(), api_latency(), api_memory(), api_memory_add(), api_memory_edit(), api_memory_forget() (+6 more)
...
Cohesion: 0.12
Nodes (27): api_audit(), api_clear_finished_sessions(), api_recent_commands(), api_state(), _build_state(), clear_finished_sessions(), _connect(), _db_path() (+19 more)
...
Cohesion: 0.12
Nodes (32): answer(), has_open_question(), on_stranger_arrived(), on_stranger_left(), Handle a reply to an open question. Returns the reply to send back, or None if…, True when each held reminder should also be texted to the owner., should_forward(), state() (+24 more)

### Community 96 - "main"
Cohesion: 0.06
Nodes (40): _acquire_single_instance_lock(), block_samples(), _dashboard_kill_background_task(), _face_release_held_notifications(), api_key(), _gemini_live_problem(), model_name(), _hold_modes() (+32 more)
...
Cohesion: 0.05
Nodes (51): _apply_memory_db_pragmas(), _background_tasks_dir(), cancel_reminder(), _catastrophic_reason(), _check_background_tasks(), _check_due_reminders(), _count_running_background_tasks(), _create_memory_tables() (+43 more)

### Community 117 - "jarvis_sleep_mode.py"
Cohesion: 0.11
Nodes (19): _cancel_media_autopause(), _dark_mode_is_on(), _db_path(), _endpoint_volume_call(), _get_volume(), guided_breathing_steps(), is_whitelisted_sender(), _log_duration_to_memory() (+11 more)
...
Cohesion: 0.18
Nodes (24): _connect(), disable(), enable(), _get_state(), Connection, kind='nap' runs the exact same mode (quiet notifications, mail take-over, dark…, save_digest(), _set_state() (+16 more)
...
Cohesion: 0.07
Nodes (32): _briefing_fetchers(), failed(), pending(), reminders(), system(), briefing_report(), _claude_in_cooldown(), _current_command_source() (+24 more)
...
### Community 126 - "_face_unavailable"
Cohesion: 0.22
Nodes (9): api_face_away(), api_face_delete(), api_face_delete_snapshots(), api_face_events(), api_face_export(), api_face_pause(), api_face_snapshot_image(), api_face_snapshots() (+1 more)

### Community 129 - "speak_text"
Cohesion: 0.06
Nodes (34): _away_warn(), _collapse_paths_for_speech(), _dashboard_approve_pending(), _sink(), _execute_confirmed_action(), _humanize_path_for_speech(), _pcm_seconds(), _play_pcm_bytes() (+26 more)
...
Cohesion: 0.04
Nodes (64): HTMLParser, _append_history(), _build_sleep_digest(), enabled(), JARVIS_<LAYER>_CACHE env flag, default on. Read on every call so it can be…, record(), stable_hash(), _cached_tools() (+56 more)

### Community 136 - "_broadcast_theme_change"
Cohesion: 0.67
...
### Community 137 - "flush_pending_notifications"
Cohesion: 0.25
Nodes (8): flush_pending_notifications(), holding_reminders(), True while due reminders must be held (not spoken, no toast)., True after the owner said "no": reminders speak even with a stranger in view., reminders_allowed(), Speaks any notifications queued while the user was busy. Called at the start of…, Should a due reminder be held (no speech, no toast) right now? The owner's…, _reminders_held_now()

### Community 139 - "self_check_report"
Cohesion: 0.11
Nodes (17): _claude_failure_reason(), _claude_live_problem(), _dashboard_get_services_status(), _fish_audio_synthesize(), _load_mcp_server_configs(), _mcp_servers_config_path(), Request, urlopen(timeout=...) is supposed to bound the whole call, but a wedged TLS… (+9 more)
...
### Community 151 - "_synthesize_and_cache"
Cohesion: 0.25
Nodes (11): _face_greet(), Runs the Deepgram REST -> Fish -> Piper synth cascade (skipping engines that…, Spoken greeting when the owner's face appears. Skipped (not queued: a stale…, fish_audio_prosody_overrides(), is_active(), Piper SynthesisConfig kwargs for calmer speech while Sleep Mode is active, or…, Fish Audio's prosody equivalent of tts_overrides() above — same calmer/quieter-…, tts_overrides() (+3 more)
...
- **Why does `_execute_tool_impl()` connect `_execute_tool_impl` to `speak_text`, `test_sleep.py`, `FileWatcher`, `jarvis.py`, `run_agent_loop`, `_autonomy_callbacks`, `test_dev_features.py`, `flush_pending_notifications`, `jarvis_memory_enhance.py`, `jarvis_guest_reminders.py`, `self_check_report`, `jarvis_dynamic_tools.py`, `jarvis_roblox.py`, `jarvis_workflow.py`, `jarvis_proactive.py`, `queue_or_deliver_notification`, `jarvis_weather.py`, `jarvis_vibes.py`, `download_image`, `jarvis_autonomy_organise.py`, `Path`, `test_qol.py`, `test_restart.py`, `test_hardening.py`, `jarvis_billing.py`, `jarvis_tech_understanding.py`, `is_paused`, `jarvis_face.py`, `jarvis_autonomy_skills.py`, `test_face.py`, `build_system_blocks`, `_rows`, `main`, `_db`, `jarvis_focus.py`, `jarvis_task_scheduler.py`, `_memory_db_connect`, `jarvis_sleep_mode.py`, `enable`, `test_guest_reminders.py`, `_briefing_fetchers`?**
  _High betweenness centrality (0.069) - this node is a cross-community bridge._
- **Why does `_future()` connect `_on` to `test_autonomy.py`, `_mail_answer`, `Fake`, `timedelta`, `RuntimeError`, `_tools`?**
