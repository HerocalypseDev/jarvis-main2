...
## Corpus Check
- 96 files · ~237,982 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 6 file(s) not represented in the graph (top: (none) 1, .vbs 1, .css 1)

## Summary
- 3421 nodes · 7382 edges · 166 communities (145 shown, 18 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 413 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ae3861c0`
...
Cohesion: 0.10
Nodes (14): db(), jarvis(), _log_kind(), fixture, Tests for sleep trend stats and the wake-up digest. Run with: python -m pytest…, A .env setting read at import time (sleep goal) must reach the modules…, test_digest_important_first_then_lighter_note(), test_disable_logs_duration_to_memory_only_when_long_enough() (+6 more)
...
Cohesion: 0.04
Nodes (79): _acquire_single_instance_lock(), _away_warn(), block_samples(), _bytes_to_gb(), _bytes_to_mb(), check_system_health(), _choose_input_device(), _chrome_executable() (+71 more)

### Community 7 - "_claude_request"
Cohesion: 0.05
Nodes (48): system(), _build_sleep_digest(), _claude_failure_reason(), _claude_in_cooldown(), _claude_live_problem(), _claude_request(), _claude_stream_first_round(), _speak_ready() (+40 more)
...
Cohesion: 0.06
Nodes (53): call(), convert_messages(), convert_tools(), from_response(), get_provider(), model_name(), _next_model(), Path (+45 more)
...
Cohesion: 0.13
Nodes (18): Short, unambiguous volume commands ("volume up", "turn it down", "mute",…, _cancel_media_autopause(), _dark_mode_is_on(), _endpoint_volume_call(), _get_volume(), guided_breathing_steps(), is_whitelisted_sender(), _log_duration_to_memory() (+10 more)
...
### Community 31 - "Path"
Cohesion: 0.11
Nodes (23): _cleanup_old_logs(), _dashboard_get_services_status(), _fmt_gb(), get_large_files_report(), _load_mcp_server_configs(), _mcp_servers_config_path(), _memory_db_path(), _print_large_files_breakdown() (+15 more)
...
Cohesion: 0.08
Nodes (33): _dictate(), _foreground_window(), _get_active_window_title(), _grab_appshot(), _grab_dictation(), handle_voice_command(), _handle_voice_command_impl(), _inflight_enter() (+25 more)
...
Cohesion: 0.15
Nodes (13): handle_text_command(), fixture, QOL pass (2026-09-23): Claude->Gemini failover, self-check, timers/stopwatch,…, _reset(), test_cleanup_old_logs(), test_hands_free_yes_never_confirms_a_staged_action(), test_repeat_and_last_reply_tracking(), test_selected_text_never_picks_a_fast_path() (+5 more)
...
Cohesion: 0.10
Nodes (49): describe_presence(), enabled(), _fresh(), group_safe(), group_safe_suppress(), list_snapshots(), poll_once(), _Presence (+41 more)
...
### Community 66 - "_db"
Cohesion: 0.12
Nodes (26): away_enabled(), away_status(), _db(), get_setting(), _greeting_text(), invalidate_profile_cache(), _maybe_greet(), Connection that commits on success and is always closed (sqlite3's own `with`… (+18 more)
...
Cohesion: 0.06
Nodes (32): is_paused(), True while the Windows lock screen (secure desktop) is up: the camera is off-…, _session_locked(), jarvis(), parametrize, Tests for jarvis_face (Phase 1). No real camera, model or network is ever…, Goes through handle_text_command -> _execute_tool (not _execute_impl): also…, Import jarvis in a fresh process with JARVIS_FACE_ENABLED set explicitly, so… (+24 more)
...
Cohesion: 0.06
Nodes (58): Exception, away_grace_s(), _away_reset(), _away_step(), away_warn_s(), _camera_failed(), _camera_ok(), _consent_summary() (+50 more)
...
### Community 96 - "_briefing_fetchers"
Cohesion: 0.18
Nodes (8): _briefing_fetchers(), failed(), pending(), reminders(), briefing_report(), _dashboard_get_pending(), Read-only snapshot of _pending_action for the dashboard's top approval bar., Morning briefing ("morning") or needs-you summary ("urgent"): {"sections":…
...
Cohesion: 0.09
Nodes (30): _connect(), _data_dir(), _DataBlob, _db_path(), download_models(), _dpapi(), blob(), _encrypt() (+22 more)
...
Cohesion: 0.04
Nodes (66): _append_history(), _apply_memory_db_pragmas(), cancel_reminder(), _catastrophic_reason(), click_at(), _contains_secret(), _count_running_background_tasks(), _create_memory_tables() (+58 more)

### Community 117 - "run_agent_loop"
Cohesion: 0.07
Nodes (44): _autonomy_callbacks(), _autonomy_run_agent(), enabled(), is_self_contained(), normalize_text(), Small, local-only cache helpers shared by jarvis.py: an env-flag reader,…, JARVIS_<LAYER>_CACHE env flag, default on. Read on every call so it can be…, Lowercase, punctuation-stripped, whitespace-collapsed — so 'System status?' and… (+36 more)
...
Cohesion: 0.06
Nodes (40): _collapse_paths_for_speech(), _current_command_source(), _dashboard_approve_pending(), _sink(), notify(), _dashboard_reject_pending(), _deterministic_intent_reply(), _execute_confirmed_action() (+32 more)
...
Cohesion: 0.09
Nodes (29): _face_greet(), _fish_audio_synthesize(), Queue non-urgent notifications while Focus Mode is on; urgent ones always get…, should_suppress(), _pcm_seconds(), _play_pcm_bytes(), Cloud-latency pass, Phase B: gates Deepgram's WebSocket speak API (audio plays…, Silent cache lookup (no hit/miss stats recorded — the caller decides… (+21 more)

### Community 130 - "_grab_selection"
Cohesion: 0.28
...
Cohesion: 0.14
Nodes (18): _connect(), _db_path(), _hhmm(), _period_stats(), Connection, Path, Stats over the `days` calendar days ending at end_day (inclusive): tracked…, Read-only sleep trends for the dashboard, from sleep_log alone (no new tables).… (+10 more)

### Community 135 - "classify_intent"
Cohesion: 0.25
...
### Community 136 - "open_cursor_window"
Cohesion: 0.24
...
Cohesion: 0.04
Nodes (77): AbstractEventLoop, _autonomy_calendar_events(), _autonomy_create_event(), _autonomy_inbound(), _autonomy_mail_hook(), _background_tasks_dir(), calendar(), mail() (+69 more)
...
### Community 145 - "set_away"
Cohesion: 0.20
Nodes (8): Why enroll/delete must not run for this command source, or None if it may., Away-mode switch. Turning it ON is always fine (it only adds protection) but…, refuse_reason(), set_away(), away(), Records lock / warning / stranger hook calls in order., _Rec, test_refuse_reason_gates()
...
- **Why does `_execute_tool_impl()` connect `_execute_tool_impl` to `enable`, `speak_text`, `jarvis_window_control.py`, `FileWatcher`, `jarvis.py`, `_claude_request`, `jarvis_memory_enhance.py`, `queue_or_deliver_notification`, `stats_summary`, `jarvis_sleep_mode.py`, `jarvis_guest_reminders.py`, `jarvis_workflow.py`, `jarvis_dynamic_tools.py`, `set_away`, `jarvis_proactive.py`, `jarvis_weather.py`, `download_image`, `jarvis_autonomy_organise.py`, `Path`, `_handle_voice_command_impl`, `test_restart.py`, `poll_once`, `jarvis_billing.py`, `_db`, `jarvis_tech_understanding.py`, `test_face.py`, `jarvis_autonomy_skills.py`, `enroll`, `jarvis_focus.py`, `build_system_blocks`, `_rows`, `_briefing_fetchers`, `test_dev_features.py`, `jarvis_task_scheduler.py`, `run_agent_loop`, `test_guest_reminders.py`, `_handle_text_command_impl`?**
