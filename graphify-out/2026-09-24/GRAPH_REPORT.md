...
Cohesion: 0.05
Nodes (62): _autonomy_inbound(), _autonomy_mail_hook(), _bytes_to_gb(), _bytes_to_mb(), check_system_health(), _choose_input_device(), _chrome_executable(), _craft_system_status_summary() (+54 more)

### Community 7 - "_claude_request"
Cohesion: 0.04
Nodes (54): _briefing_fetchers(), failed(), pending(), reminders(), system(), briefing_report(), _build_sleep_digest(), _claude_in_cooldown() (+46 more)
...
Cohesion: 0.06
Nodes (52): call(), convert_messages(), convert_tools(), from_response(), get_provider(), _next_model(), Path, Request (+44 more)
...
### Community 31 - "Path"
Cohesion: 0.07
Nodes (32): _claude_failure_reason(), _claude_live_problem(), _cleanup_old_logs(), _dashboard_get_services_status(), _fmt_gb(), get_large_files_report(), _load_mcp_server_configs(), _mcp_servers_config_path() (+24 more)
...
### Community 35 - "main"
Cohesion: 0.05
Nodes (51): _acquire_single_instance_lock(), _away_warn(), block_samples(), _dashboard_get_sleep(), _dashboard_get_usage(), _dashboard_kill_background_task(), _dictate(), handle_text_command() (+43 more)
...
### Community 69 - "jarvis_briefing.py"
Cohesion: 0.16
Nodes (20): calendar_items(), compose(), deadline_items(), _hm(), mail_items(), datetime, Morning briefing v2 and "what's urgent?" (2026-09-23). One composition used by…, Calendar MCP (@cocal/google-calendar-mcp) list-events JSON -> "9:30 AM Standup"… (+12 more)

### Community 72 - "test_face.py"
Cohesion: 0.05
Nodes (42): is_paused(), True while the Windows lock screen (secure desktop) is up: the camera is off-…, recent_events(), _session_locked(), away(), fx(), jarvis(), fixture (+34 more)
...
Cohesion: 0.06
Nodes (84): Exception, _already_enrolled(), _already_seen_recently(), away_enabled(), away_grace_s(), _away_reset(), away_status(), _away_step() (+76 more)
...
### Community 77 - "enroll"
Cohesion: 0.09
Nodes (38): _clean_name(), delete(), delete_by_id(), describe_profiles(), enroll(), list_profiles(), Why enroll/delete must not run for this command source, or None if it may., (role, None) to enroll with, or (None, reason). The first person is always the… (+30 more)
...
Cohesion: 0.09
Nodes (17): calibrate(), camera_index(), CameraUnavailable, _capture_and_analyze(), _get_engine(), _open_camera(), Open the camera, grab one frame, release it. Returns (observations, mean, std,…, Dry run of the enrollment liveness check that stores NOTHING (no profile, no… (+9 more)
...
Cohesion: 0.09
Nodes (32): _connect(), _crop_jpeg(), _data_dir(), _DataBlob, _db_path(), download_models(), _dpapi(), blob() (+24 more)
...
Cohesion: 0.06
Nodes (48): _apply_memory_db_pragmas(), _background_tasks_dir(), cancel_reminder(), _catastrophic_reason(), _check_background_tasks(), _count_running_background_tasks(), _create_memory_tables(), _delegate_child_env() (+40 more)

### Community 117 - "run_agent_loop"
Cohesion: 0.05
Nodes (57): _append_history(), enabled(), is_self_contained(), normalize_text(), Small, local-only cache helpers shared by jarvis.py: an env-flag reader,…, JARVIS_<LAYER>_CACHE env flag, default on. Read on every call so it can be…, Lowercase, punctuation-stripped, whitespace-collapsed — so 'System status?' and…, record() (+49 more)
...
Cohesion: 0.06
Nodes (43): _collapse_paths_for_speech(), _current_command_source(), _dashboard_approve_pending(), _sink(), notify(), _dashboard_reject_pending(), _deterministic_intent_reply(), _execute_confirmed_action() (+35 more)
...
Cohesion: 0.07
Nodes (29): _autonomy_calendar_events(), _autonomy_callbacks(), _autonomy_create_event(), _autonomy_run_agent(), calendar(), _calendar_events_raw(), _commands_in_flight(), create_reminder() (+21 more)
...
Cohesion: 0.05
Nodes (56): AbstractEventLoop, mail(), _budget_check(), _check_due_reminders(), _chief_tick(), _ensure_mcp_loop(), _env_float(), _forward_held_reminders() (+48 more)
...
- **Why does `_execute_tool_impl()` connect `_execute_tool_impl` to `enable`, `_autonomy_callbacks`, `test_sleep.py`, `FileWatcher`, `jarvis.py`, `_claude_request`, `jarvis_memory_enhance.py`, `queue_or_deliver_notification`, `jarvis_vibes.py`, `jarvis_guest_reminders.py`, `jarvis_workflow.py`, `jarvis_dynamic_tools.py`, `jarvis_proactive.py`, `jarvis_weather.py`, `download_image`, `jarvis_autonomy_organise.py`, `Path`, `main`, `test_restart.py`, `poll_once`, `test_hardening.py`, `jarvis_billing.py`, `jarvis_tech_understanding.py`, `test_face.py`, `jarvis_face.py`, `jarvis_autonomy_skills.py`, `enroll`, `jarvis_focus.py`, `build_system_blocks`, `_rows`, `jarvis_roblox.py`, `test_dev_features.py`, `jarvis_task_scheduler.py`, `_memory_db_connect`, `run_agent_loop`, `test_guest_reminders.py`, `_handle_text_command_impl`?**
